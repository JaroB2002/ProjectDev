<?php
    class Favorite{
        ...
        public function save(){
            $conn = Db::getInstance();
            $statement = $conn->prepare("insert into favorites (prompts_id, username) values (:prompts_id, :username)");
        ...
        {
                $conn = Db::getInstance();
                $statement = $conn->prepare("SELECT * FROM favorites WHERE prompts_id = :prompts_id AND username = :username");
        ...
        {
                $conn = Db::getInstance();
                $statement = $conn->prepare("DELETE FROM favorites WHERE prompts_id = :prompts_id AND username = :username");
                $statement->bindValue(":prompts_id", $prompt_id);
                $statement->bindValue(":username", $_SESSION['username']);
                $statement->execute();
        }
