            ...
            <div class="my-5 mx-5 bg-stone-700 px-8 py-8 rounded max-w-sm">
                ...
                <div>
                    <?php if (isset($errorMessage) && $_GET["buy"] == $prompt["id"]): ?>
                        <div class="error-message text-red-500" id="<?php echo 'error-' . htmlspecialchars($prompt["id"]); ?>">
                    ...
                    <div class="mt-5">
                        <a href="#" data-id="<?php echo htmlspecialchars($prompt['id']); ?>" class="like bg-fadedpurple px-5 py-3 rounded font-semibold ml-5 mt-5"><?php if(Like::getAll($prompt['id']) == true) { echo 'Unlike '; } else { echo 'Like ';}?> <span class='likes' id="likes"><?php echo $likes->getLikes($prompt['id']) ?> people like this</span> </a>
                    </div>
                    <div class="mt-8">
                        <a href="#" data-id="<?php echo htmlspecialchars($prompt['id']); ?>" class="favorite bg-fadedpurple px-5 py-3 rounded font-semibold ml-5 mt-5"><?php if(Favorite::getAll($prompt['id']) == true) { echo 'remove from favorites '; } else { echo 'add to favorites ';}?></a>
                    </div>
                </div>
            </div>
